#!/usr/bin/env Rscript
# ==============================================================================
# VALIDITÄTSANALYSEN: SUBGRUPPEN - COPING-SKALEN (NEU)
# ==============================================================================
#
# Demographische Subgruppenanalysen für alle 5 Coping-Skalen:
# - Nach Geschlecht (für alle 5 Skalen)
# - Nach Bildung (für alle 5 Skalen)
# - Nach Alter (für alle 5 Skalen)
# - Nach Beschäftigung (für alle 5 Skalen)
#
# Verwendet Fisher's Z-Tests für Gruppenvergleiche
#
# HINWEIS: Diese Analysen waren im Original-Skript NICHT vorhanden
#          und werden hier neu hinzugefügt!
#
...
coping_labels <- c(
  "Aktives Coping", "Drogen-Coping", "Positive Neubewertung",
  "Soziales Coping", "Religiöses Coping"
)
...
gender_results_coping <- list()

for (i in seq_along(coping_skalen)) {
  skala <- coping_skalen[i]
  ...
  for (krit in validitaetskriterien) {
    ...
    gender_results_coping[[key]] <- list(
      skala = skala,
      label = label,
...
for (i in seq_along(coping_skalen)) {
  ...
  for (krit in validitaetskriterien) {
    ...
    key <- paste(skala, krit, sep = "_")
    bildung_results_coping[[key]] <- list(
      skala = skala,
      label = label,
...
beschaeftigung_results_coping <- list()
...
for (i in seq_along(coping_skalen)) {
  ...
  for (krit in validitaetskriterien) {
    ...
    if (length(cors) >= 2) {
      ...
      n2 <- sum(data_beschaeftigung$Beschäftigung_gruppiert == gruppe2)
      fisher <- fisher_z_test(cors[[gruppe1]]$estimate, n1, cors[[gruppe2]]$estimate, n2)
      beschaeftigung_results_coping[[key]] <- list(
        skala = skala,
        label = label,
        cors = cors,
        fisher_z = fisher$z,
...
cat("✓ Subgruppenanalysen für ALLE 5 Coping-Skalen abgeschlossen!\n\n")
cat("Diese umfangreichen Analysen waren im Original-Skript nicht enthalten.\n")
cat("Plots werden in 08_create_subgroup_plots.R erstellt.\n\n")
cat("Führen Sie als nächstes aus: 08_create_subgroup_plots.R\n\n")
